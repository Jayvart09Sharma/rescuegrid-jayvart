...
// reveal wave sweeps out from the centre and rebuilds it into a solid, coloured model (buildings rise
...
const CORE = 50
const OUTER = 110
/** Load reveal: starts after WAVE_DELAY s, front moves WAVE_SPEED units/s, WAVE_BAND units wide. */
...
uniform float uWave;
float waveAt(vec2 xz) { return smoothstep(uWave, uWave - ${WAVE_BAND.toFixed(1)}, length(xz)); }
...
export function MeshCity() {
  ...
  useFrame((_, dt) => {
    ...
    const waveAt = (x: number, z: number) => {
      const k = Math.min(1, Math.max(0, (wave - Math.hypot(x, z)) / WAVE_BAND))
      return k * k * (3 - 2 * k)
    }
