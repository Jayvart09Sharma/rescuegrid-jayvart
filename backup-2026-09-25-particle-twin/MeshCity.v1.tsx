...
// graph status) but drawn as lit meshes: everything starts as a blueprint grid and is rebuilt into a
// solid, textured model once a drone camera has covered it. Buildings rise floor by floor behind a
// scan band. Nothing here decides what's true; it only draws the store.
// Switch back to the point-cloud twin with ?twin=particles; ?reveal=all shows the whole city built.
...
const FORCE_REVEAL = typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('reveal') === 'all'
/** Low golden-hour sun, from the south-west; the sky, the light and the shadows all use it. */
export const SUN = new THREE.Vector3(-70, 38, 55)
...
float coverAt(vec2 xz) { return max(uForce, texture2D(uCover, (xz + uExtent) / (2.0 * uExtent)).r); }
...
vBuild = uForce > 0.5 ? 1.0 : (rv < -0.5 ? 0.0 : clamp((uTime - rv) / 1.8, 0.0, 1.0));
vCollapse = 0.0;
if (st.g > -0.5) {
  float tc = uTime - st.g;
...
// facade
vec3 facade = seed < 0.22 ? vec3(0.50, 0.48, 0.44)       // concrete
            : seed < 0.45 ? vec3(0.42, 0.20, 0.14)       // brick
            : seed < 0.65 ? vec3(0.62, 0.53, 0.40)       // sandstone
            : seed < 0.82 ? vec3(0.36, 0.40, 0.44)       // grey panel
            :               vec3(0.70, 0.68, 0.64);      // white render
bool tower = H > 12.0;
if (tower) facade = vec3(0.16, 0.22, 0.27);
...
vec3 glass = vec3(0.07, 0.10, 0.13) + vec3(0.10, 0.12, 0.13) * vnoise(vec2(u * 0.3, v * 0.5) + seed * 7.0);
...
float kx = clamp(floor((p.x + 50.0) / 10.0 + 0.5), 0.0, 10.0);
float kz = clamp(floor((p.y + 50.0) / 10.0 + 0.5), 0.0, 10.0);
float dx = p.x - (-50.0 + kx * 10.0);
float dz = p.y - (-50.0 + kz * 10.0);
float hwx = 1.1;
float hwz = abs(-50.0 + kz * 10.0) < 0.1 ? 1.6 : 1.1;     // Main St is wider
bool inCity = abs(p.x) < 51.7 && abs(p.y) < 51.7;
bool onV = inCity && abs(dx) < hwx && abs(p.y) <= 50.0 + hwz;
bool onH = inCity && abs(dz) < hwz && abs(p.x) <= 50.0 + hwx;
bool road = onV || onH;
bool walk = !road && inCity && (abs(dx) < hwx + 0.45 || abs(dz) < hwz + 0.45);
vec2 blk = -45.0 + 10.0 * floor((p + 50.0) / 10.0);
bool park = inCity && (distance(blk, vec2(-5.0, 15.0)) < 0.1 || distance(blk, vec2(25.0, 25.0)) < 0.1 || distance(blk, vec2(-15.0, 25.0)) < 0.1);
...
vec3 grass = mix(vec3(0.16, 0.24, 0.08), vec3(0.30, 0.31, 0.13), n2) * (0.85 + 0.3 * n1);
vec3 col;
float rough = 0.95;
if (road) {
  col = vec3(0.105, 0.11, 0.115) * (0.8 + 0.35 * n3) * (0.9 + 0.2 * n2);
  rough = 0.8;
  bool inter = onV && onH;
  float a = onH ? p.x : p.y;
  float q = onH ? dz : dx;
  ...
  col = vec3(0.44, 0.43, 0.41) * (0.85 + 0.25 * n3);
  ...
  col = mix(vec3(0.36, 0.35, 0.33), grass, step(0.72, n2)) * (0.85 + 0.25 * n3);
} else {
  vec3 dry = vec3(0.36, 0.31, 0.19);
...
float c = coverAt(p);
float prog = smoothstep(0.05, 0.45, c) * uIntro;
float fade = smoothstep(95.0, 55.0, length(p));
float ripple = sin(p.x * 1.3 + uTime * 1.4 + sin(p.y * 2.0)) * 0.5 + 0.5;
diffuseColor.rgb = mix(vec3(0.003, 0.01, 0.015), vec3(0.035, 0.085, 0.09) * (0.85 + 0.3 * vnoise(p * 0.4 + uTime * 0.05)), prog);
...
function onRoadOrRiver(x: number, z: number, margin: number) {
  ...
  const dx = Math.abs(x - (-50 + Math.round((x + 50) / 10) * 10))
  const dz = Math.abs(z - (-50 + Math.round((z + 50) / 10) * 10))
...
function Trees({ uniforms, boxes }: { uniforms: Uniforms; boxes: Box[] }) {
  ...
  const spots = useMemo(() => {
    ...
    // parks and the school field
    for (const [cx, cz] of [[-5, 15], [25, 25], [-15, 25]]) {
      for (let k = 0; k < 26; k++) {
    ...
    // odd trees in city lots, woods and hedgerows outside town
    for (let k = 0; k < 2600 && out.length < 1400; k++) {
      const x = (r() - 0.5) * 230
      const z = (r() - 0.5) * 230
      const inTown = Math.abs(x) < 52 && Math.abs(z) < 52
      if (inTown ? r() > 0.08 : r() > 0.55) continue
...
function Cars({ uniforms }: { uniforms: Uniforms }) {
  ...
  const cars = useMemo(() => {
    ...
    for (let k = 0; k < 900 && out.length < 240; k++) {
      const vertical = r() < 0.5
      const line = -50 + Math.floor(r() * 11) * 10
      const along = -49 + r() * 98
      const hw = !vertical && line === 0 ? 1.6 : 1.1
      const side = r() < 0.5 ? -1 : 1
      const off = line + side * (hw - 0.4)
      const [x, z] = vertical ? [off, along] : [along, off]
      const cross = vertical ? Math.abs(z - (-50 + Math.round((z + 50) / 10) * 10)) : Math.abs(x - (-50 + Math.round((x + 50) / 10) * 10))
...
function Lighting() {
  ...
  useEffect(() => {
    ...
    cam.left = -75; cam.right = 75; cam.top = 75; cam.bottom = -75; cam.near = 1; cam.far = 260
    cam.updateProjectionMatrix()
  }, [])
  return (
    <>
      <Sky distance={450} sunPosition={SUN.toArray()} turbidity={7} rayleigh={1.6} mieCoefficient={0.006} mieDirectionalG={0.85} />
      <hemisphereLight args={['#bcd0e6', '#4a3f33', 0.9]} />
      <directionalLight
        ref={sun}
...
export function MeshCity() {
  ...
  useFrame((_, dt) => {
    ...
    for (const br of bridges) {
      ...
      const c = sampleCover(img, size, br.a[0], (br.a[1] + br.b[1]) / 2)
