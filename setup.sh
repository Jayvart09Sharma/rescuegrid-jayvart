...
# Applied on every run; each key's existing (possibly commented) declaration is replaced so nothing is declared twice.
...
# initial password only on a store that has never started (neo4j / rescuegrid)
[ -f "$B/neo4j/neo4j/data/dbms/auth.ini" ] || JAVA_HOME="$B/neo4j/jdk" "$B/neo4j/neo4j/bin/neo4j-admin" dbms set-initial-password rescuegrid >/dev/null
