...
4. Dry-run it first without touching the shared graph: start a second vision instance on :8092 with no bus
   (`cd backend/vision && .venv/bin/python -m uvicorn server:app --port 8092`)
...
cd backend/reasoning && NEO4J_URI=bolt://127.0.0.1:7688 .venv/bin/python -c "from rescuegrid.graph import GraphStore; g=GraphStore(); g.reset()"
...
```python
import sys; sys.path.insert(0, "backend/scenario")
