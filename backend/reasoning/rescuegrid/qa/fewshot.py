...
EXAMPLES_V2: list[dict] = [
    ...
    {"q": "Give me the updates from the past 10 minutes.", "tags": "changed recent last minutes events happened new latest updates", "cypher":
     "MATCH (e:Event)-[:ABOUT]->(n) WHERE e.timestamp >= $now - duration({minutes: 10})\n"
    ...
    {"q": "Is there a passable route for Engine 7 to the shelter?", "tags": "reach route path get to hospital roads connect drive", "cypher":
     "MATCH (t:Team {id: 'Team-Engine7'}), (start:Road) WHERE start.location IS NOT NULL\n"
     "WITH t, start ORDER BY point.distance(t.location, start.location) ASC LIMIT 1\n"
     "MATCH (f:Facility {facility_type: 'shelter'})-[:ON_ROAD]->(dest:Road)\n"
    ...
    {"q": "Which source said Main Street is blocked, and how sure was it?", "tags": "who reported report evidence confidence source provenance collapse", "cypher":
     "MATCH (e:Event)-[:ABOUT]->(n:Entity {id: 'Road-Main'}) WHERE e.claim = 'blocked'\n"
    ...
    {"q": "Anything from the drones in the past 4 minutes?", "tags": "radio drone sensor gps field report said see reported source last minutes", "cypher":
     "MATCH (e:Event)-[:ABOUT]->(n) WHERE e.source = 'drone_vision' AND e.timestamp >= $now - duration({minutes: 4})\n"
    ...
    {"q": "Does the seismic zone currently contain a responder?", "tags": "units near inside hazard zone gas leak", "cypher":
     "MATCH (t:Team)-[n:NEAR]->(h:Hazard) WHERE n.active = true AND h.status = 'active' AND h.hazard_type = 'seismic'\n"
    ...
    {"q": "List units no more than 150 metres from the shelter.", "tags": "within metres meters radius distance of hospital units", "cypher":
     "MATCH (f:Facility {facility_type: 'shelter'}), (t:Team) WHERE t.location IS NOT NULL AND point.distance(t.location, f.location) <= 150\n"
    ...
    {"q": "Which entities have disputed status right now?", "tags": "conflict conflicting disagree contradict dispute sources versus", "cypher":
    ...
    {"q": "Show the events logged from 14:01 up to 14:02.", "tags": "between and from to happened events time range window", "cypher":
     "MATCH (e:Event)-[:ABOUT]->(n) WHERE e.timestamp >= datetime('2026-09-25T14:01:00Z') AND e.timestamp < datetime('2026-09-25T14:02:00Z')\n"
    ...
    {"q": "Show every active assignment with its unit and target.", "tags": "assigned assignment tasked units staged at", "cypher":
     "MATCH (t:Team)-[a:ASSIGNED_TO]->(x:Entity) WHERE a.active = true\n"
    ...
    {"q": "Give me the current picture for Building 22.", "tags": "status of is latest what happened to building road facility", "cypher":
     "MATCH (n:Entity {id: 'Building-22'})\n"
